...
export const filmAPI = api.injectEndpoints({
  endpoints: (build) => ({
    ...
    getMoviesByGenre: build.query<IMovieDetails, string>({
      query: (id) =>
        `discover/movie?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_genre=${id}`,
    }),
  }),
});
