...
export function ResponsiveDrawer({ children, header }: Props) {
  ...
  const desiredPath = currentPath.split("/")[1];
  ...
  const strongTypography = (path: string) => {
    if (!desiredPath) return "600";
    if (path.includes(desiredPath)) return "600";
    return "400";
  };
      ...
      <Stack mt={6}>
        {pages.map((page) => (
          <Link to={page.path} style={styles.link} key={page.path}>
            ...
            >
              {page.icon}
              <Typography fontSize="16px" fontWeight={strongTypography}>
                {page.text}
              </Typography>
            </Container>
          </Link>
